...
- 禁止把 `process.execArgv` 原样传递给子进程；Node 子进程参数必须使用明确白名单；
- 测试不应通过再次启动 Node 来验证可直接导入的项目逻辑，CLI 只拥有参数解析和结果输出；
- 本应秒级结束的诊断命令运行超过 10 秒且无输出时，先终止原进程，不得继续并行重试；
- 本地完整 Gate 使用 `AIGW_GATE_CONCURRENCY=1`，并优先在具有 `MemoryMax` 与 `TasksMax` 的受限 Scope 中运行；
- 子进程测试必须设置与其预期耗时相符的超时，并在失败或取消后清理拥有的进程树。
