...
for (let index = 1; index < migrations.length; index += 1) {
  if (migrations[index] <= migrations[index - 1]) failures.push("migration filenames are not monotonic");
}
...
for (let index = 0; index < migrations.length; index += 1) {
  const tag = migrations[index].replace(/\.sql$/, "");
  if (journal.entries[index]?.idx !== index || journal.entries[index]?.tag !== tag) {
    failures.push(`Drizzle journal does not match ${migrations[index]}`);
  }
...
for (const table of expectedTables) {
  if (!combined.includes(`\"${table}\"`)) failures.push(`migration set does not contain ${table}`);
}
for (const constraint of [
  ...
  if (!combined.includes(`\"${constraint}\"`)) failures.push(`migration set does not contain ${constraint}`);
}
...
for (const table of expectedTables) {
  if (!snapshotTables.has(table)) failures.push(`latest Drizzle snapshot does not contain ${table}`);
}
