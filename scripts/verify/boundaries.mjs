...
for (const file of gatewayFiles) {
  ...
  for (const match of source.matchAll(importPattern)) {
    ...
    if (!resolved.startsWith(gatewayRoot)) continue;
  ...
  if (fromLayer === "data-plane") {
    for (const token of ["Response.clone(", ".tee(", "streamSSE("]) {
      if (source.includes(token)) failures.push(`${relative} contains forbidden data-plane token ${token}`);
    }
...
for (const file of webFiles) {
  ...
  for (const match of source.matchAll(importPattern)) {
    ...
    if (resolved === null || !resolved.startsWith(webRoot)) continue;
...
function checkControlFeatureIsolation(from, target, output) {
...
function checkWebBoundary(from, target, output) {
...
async function resolveWebImport(fromFile, specifier) {
  let base;
...
async function findTypeScript(directory, options) {
  const results = [];
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (matchesExtension && (options.includeTests || !isTest)) results.push(fullPath);
  }
  return results;
}
