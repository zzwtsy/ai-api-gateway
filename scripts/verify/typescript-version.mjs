...
for (const manifestPath of manifests) {
  ...
  for (const section of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"]) {
    ...
    for (const forbidden of ["@typescript/native", "@typescript/typescript6"]) {
      if (forbidden in entries) findings.push(`${path.relative(root, manifestPath)} uses forbidden ${forbidden}`);
    }
    if ("typescript" in entries) {
      const relative = path.relative(root, manifestPath);
      if (relative !== "package.json") findings.push(`${relative} declares its own TypeScript version`);
      if (entries.typescript !== "6.0.3") findings.push(`${relative} must pin TypeScript exactly to 6.0.3`);
    }
  }
}
...
async function findManifests(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) results.push(...await findManifests(fullPath));
