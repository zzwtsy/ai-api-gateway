...
import { gatesFor, allowedGateModes } from "../gates/definitions.mjs";
import { aggregatePassed, runGateGraph, validateGateGraph } from "../gates/gate-runner.mjs";

const gate = (id, needs = [], after = []) => ({
  id,
  label: id,
  command: "node",
  args: [],
...
test("invalid executor status is converted into a failed Gate result", async () => {
  const results = await runGateGraph([gate("invalid")], {
    execute: async () => ({ status: "unknown", exitCode: 0 }),
  });
