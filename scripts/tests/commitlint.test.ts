...
async function lint(message: string): Promise<number> {
  ...
  return await new Promise<number>((resolve, reject) => {
    const child = spawn(path.join(root, "node_modules/.bin/commitlint"), ["--config", "commitlint.config.ts", "--edit", messagePath], {
      cwd: root,
      stdio: "ignore",
    });
