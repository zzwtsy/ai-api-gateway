...
export async function collectProjectVersionViolations(repositoryRoot: string): Promise<string[]> {
...
export async function setProjectVersion(repositoryRoot: string, targetVersion: string) {
  ...
  const updates: { absolute: string; updated: string }[] = [];
  for (const item of projectVersionProjections) {
...
function collectMatches(source: string, pattern: RegExp): string[] {
  return [...source.matchAll(clonePattern(pattern))].flatMap(match => match[2] === undefined ? [] : [match[2]]);
}
...
function projection(relativePath: string, pattern: RegExp): Readonly<VersionProjection> {
