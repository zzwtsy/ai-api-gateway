...
try {
  ...
  const received = await fetch("http://127.0.0.1:4010/received").then((value) => value.json());
  if (received.authorization !== "Bearer mock-provider-key" || JSON.stringify(JSON.parse(received.body)) !== JSON.stringify(body)) {
  ...
  const detail = await fetch(`http://127.0.0.1:3001/admin/api/v1/requests/${list.data[0].id}`, {
    headers: { authorization: "Bearer admin_dev_local" },
  }).then((value) => value.json());
