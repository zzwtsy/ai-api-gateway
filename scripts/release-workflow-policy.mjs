...
export function collectReleaseWorkflowViolations(source) {
  const failures = [];
  ...
  const imageOwnership = source.indexOf("scripts/release/verify-image-ownership.mjs");
...
function workflowStepContaining(source, position) {
  if (position < 0)
    return "";
...
function requireText(failures, source, expected, message) {
  if (!source.includes(expected))
    failures.push(message);
}

function requireMatch(failures, source, pattern, message) {
