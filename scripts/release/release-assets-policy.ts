...
interface CreateReleaseAssetsOptions {
...
export function parseReleaseAssetArguments(rawArgs: string[]): { version: string; check: boolean } {
...
export async function createReleaseAssets({ repositoryRoot, outputDirectory, version, allowDirty = false }: CreateReleaseAssetsOptions) {
  ...
  const specificationNames: string[] = (bundleManifest.bundles ?? []).map((bundle: { output: string }) => bundle.output);
...
async function readArchiveMetadata(archivePath: string, prefix: string): Promise<{ version?: unknown; commit?: unknown }> {
