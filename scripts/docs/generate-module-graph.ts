...
interface GraphOptions {
  ...
  sourceRoot: string;
  title: string;
  classify: (relativePath: string) => string | null;
  includeTsx: boolean;
}

export async function buildModuleGraphDocument(repositoryRoot: string = root): Promise<string> {
...
async function renderGraph(options: GraphOptions): Promise<string> {
  ...
  for (const file of files) {
    ...
    for (const match of source.matchAll(importPattern)) {
      ...
      if (target === null || !target.startsWith(options.sourceRoot))
        continue;
  ...
  const sortedEdges = [...edges.entries()]
    ...
    .sort((left, right) => {
      ...
      return fromComparison !== 0 ? fromComparison : left.to.localeCompare(right.to);
    });
  ...
  if (sortedEdges.length === 0)
    lines.push("| — | — | 当前没有跨层静态 Import | ");
  return lines.join("\n");
}

function classifyGateway(relativePath: string): string | null {
...
function classifyWeb(relativePath: string): string | null {
  const [first, second] = relativePath.split("/");
  if (first === "features")
    return `feature:${second ?? "unknown"}`;
  if (first === "routes")
    return "routes";
  if (["main.tsx", "app.tsx", "router.tsx"].includes(relativePath))
    return "application";
  return first ?? null;
}

async function resolveImport(fromFile: string, specifier: string, sourceRoot: string): Promise<string | null> {
...
async function findSourceFiles(directory: string, includeTsx: boolean): Promise<string[]> {
...
function relative(base: string, value: string): string {
