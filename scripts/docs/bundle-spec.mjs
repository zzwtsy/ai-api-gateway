...
import { resolveSourceCommit } from "./source-identity.mjs";

const root = path.resolve(import.meta.dirname, "../..");
const outputDirectory = path.join(root, ".artifacts/spec");
...
async function buildBundle(bundle, outputPath) {
  const sections = [];
  for (const sourcePath of bundle.sources) {
    ...
    if (content.length === 0) throw new Error(`Empty bundle source: ${sourcePath}`);
...
function validateBundle(bundle) {
  if (typeof bundle.id !== "string" || typeof bundle.title !== "string" || typeof bundle.output !== "string") {
    throw new Error("Invalid docs/spec-bundles.json entry");
  }
  ...
  if (seenOutputs.has(bundle.output)) throw new Error(`Duplicate bundle output: ${bundle.output}`);
  seenOutputs.add(bundle.output);
  const sourceSet = new Set(bundle.sources);
  if (sourceSet.size !== bundle.sources.length) throw new Error(`Bundle ${bundle.id} contains duplicate sources`);
}
...
function rewriteLocalLinks(text, sourceFile, outputFile) {
  return text.replace(/(!?\[[^\]]*\]\()([^)]+)(\))/g, (whole, prefix, rawTarget, suffix) => {
    const target = rawTarget.trim();
    if (/^(?:#|https?:|mailto:|data:)/.test(target) || target.includes(' "') || target.includes(" '")) return whole;
    ...
    if (relativeToRoot.startsWith("..") || path.isAbsolute(relativeToRoot)) return whole;
