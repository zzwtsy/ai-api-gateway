...
function parseCli(args: string[]): { mode: string; reportPath: string | undefined } {
  ...
  if (mode === undefined || positionals.length !== 1 || !allowedGateModes.includes(mode)) {
    throw new Error(`用法：node scripts/check.ts <${allowedGateModes.join("|")}> [--report <path>]`);
  }
