...
async function readBaselineSnapshot(root, baseline) {
  ...
  ].map(async relative => [relative, await readJson(relative)]));

  return {
    rootManifest: await readJson("package.json"),
    webManifest: await readJson("apps/web/package.json"),
    components: await readJson("apps/web/components.json"),
    designTokens: await readJson("docs/product/ux/design-tokens.json"),
    webTypeScriptConfigs,
    catalog: await readText("pnpm-workspace.yaml"),
    eslint: await readText("eslint.config.mjs"),
...
function collectToolchainConfigViolations(snapshot, baseline) {
  return [
...
function collectProjectIdentityViolations(rootManifest, baseline) {
  const errors = [];
...
function collectComponentManifestViolations(components, baseline) {
  const errors = [];
...
function collectDependencyViolations(snapshot, baseline) {
  return [
...
function collectWebDependencyViolations(webManifest, baseline) {
  const errors = [];
...
function collectRepositoryDependencyViolations(catalog, rootManifest, baseline) {
  const errors = [];
...
function expectedCatalogVersions(baseline) {
  return [
    ["@base-ui/react", "1.7.0"],
...
function collectLintAndRuntimeViolations(snapshot, baseline) {
  return [
...
function collectEslintViolations(eslint) {
  const errors = [];
...
function collectRouterViolations(snapshot, baseline) {
  const errors = [];
...
function collectPrimitiveViolations(snapshot) {
  const errors = [];
...
function collectThemeViolations(snapshot, baseline) {
  const errors = [];
...
async function collectRequiredFileViolations(root, baseline) {
  const errors = [];
...
async function collectDirectoryOwnershipViolations(root, baseline) {
  const errors = [];
...
async function collectRegistrySourceViolations(root, baseline) {
  return [
...
async function collectComponentSourceViolations(root, baseline) {
  const errors = [];
...
async function collectHookSourceViolations(root, baseline) {
  const errors = [];
...
function collectWebTypeScriptConfigViolations(configEntries) {
  const errors = [];
