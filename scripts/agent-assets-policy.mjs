...
export async function collectAgentAssetViolations(repositoryRoot, options = {}) {
...
async function readThirdPartySkills(repositoryRoot) {
  try {
    const lock = JSON.parse(await readFile(path.join(repositoryRoot, "skills-lock.json"), "utf8"));
...
function isLockedThirdPartySkill(relative, names) {
