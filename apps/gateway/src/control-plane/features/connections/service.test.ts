...
describe("ConnectionService", () => {
  it("creates an aggregate that exposes only masked Credential metadata", async () => {
    ...
    const service = new ConnectionService(repository, cipher, { now: () => now }, unusedProber);
    const created = await service.create({
      name: "测试连接",
      providerSlug: "test-provider",
      endpoint: { name: "Chat", protocol: "openai-chat", baseUrl: "https://provider.example/", requestPath: "/v1/chat/completions", authScheme: "bearer", supportsStreaming: true },
      account: { name: "主账号", billingMode: "metered" },
      credential: { name: "主 Key", secret: "provider-test-value" },
    });
  ...
  it("decrypts a bound Credential only for an explicit Probe and persists the safe result", async () => {
    ...
    const service = new ConnectionService(repository, cipher, { now: () => now }, {
      probe: async (input) => {
    ...
    expect((await repository.getById(connection.id))?.accounts[0]?.credentials[0]).toMatchObject({
      status: "healthy",
      lastSuccessAt: now,
    });
  });
  ...
  it("adds a second protocol and binds only the selected Credential", async () => {
    ...
    const updated = await service.addEndpoint(connection.id, {
      name: "Responses",
    ...
    expect(updated.accounts[0]?.credentials[0]?.endpointIds).toEqual([
      connection.endpoints[0]!.id,
      updated.endpoints[1]!.id,
    ]);
  });
  ...
  it("rejects a Credential owned by another Provider when adding an Endpoint", async () => {
    ...
    const foreignCredentialId = otherConnection.accounts[0]!.credentials[0]!.id;

    await expect(service.addEndpoint(connection.id, {
      name: "Responses",
      protocol: "openai-responses",
      ...
      authScheme: "bearer",
      supportsStreaming: true,
      credentialIds: [foreignCredentialId],
