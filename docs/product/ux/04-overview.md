...
当前概览交付逻辑请求数、连接数、最终成功率和平均延迟四项摘要，最近请求表格以及当前请求转发链路说明。时间范围、费用、趋势、Attention 和厂商健康仍属于后续交付。

当连接或 Gateway Client 任一类对象为空时，页面显示三步起步向导并导航到连接、模型和客户端页面；两类查询尚未得到成功数据时，不把 unknown 当作空列表。向导是否出现不依赖历史请求，也不建立独立的客户端持久化进度。
