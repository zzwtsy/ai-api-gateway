...
- 请求 → `connections?connectionId=connection_id&tab=accounts`；
- 模型 → `routes?model=deepseek-v4-pro`；
- 模型目录 → `models?modelBindingId=model_binding_id`；
- 客户端目录 → `clients?clientId=client_id`；
- 概览 Attention → 对应 Request、Provider 或分析筛选。

只允许链接到目标 Route 已拥有的 Search 参数。Request 的客户端筛选尚无服务端合同，因此不得用最近 50 条本地数据伪造 `requests?client=...`。
