...
- **Master–Detail（双栏工作台）**：承载请求排障、连接检视等需要持续比对目录与详情的深度工作，无遮罩伴随查阅；
- **Sheet（侧滑抽屉）**：承载客户端与模型的临时上下文详情，以及复杂全局筛选器、只读 Raw Payload 报文检查等辅助工具；详情由 URL ID 恢复，但 Sheet 不参与底层页面布局，Header 固定且 Body 在视口内滚动；禁止作为主创建表单或在单抽屉内嵌套多步骤业务状态；
